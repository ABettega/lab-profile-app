...
router.post('/upload', upload.single('profilePic'), (req, res, next) => {
  if (!req.file) {
    res.status(400).json({message: 'The selected file is not a picture!'});
    return;
  }

  User.findOneAndUpdate({_id: req.user._id}, {$set: {image: req.file.secure_url}}, {new: true})
    .then(user => res.status(200).json(user))
    .catch(err => res.status(400).json(err))
});
